/*
 developer	: 	Vagelis Mitsis - Kommigraphics
*/
...
var Main = {
    ...
    init: function () {
        ...
        tl_cookie.staggerTo(cookie.fade, 0.4, { autoAlpha: 0, ease: Power1.easeIn }, 0.02, 0);
        tl_cookie.to(cookie.bg, 0.8, { scaleY: 0, transformOrigin: "bottom left", ease: ease_custom.ease1 }, 0.6);
        tl_cookie.set(cookie.holder, { autoAlpha: 0, className: '+=remove' }, 1);
        tl_cookie.staggerTo(cookie.fade, 0.4, { autoAlpha: 0, ease: Power1.easeIn }, 0.02, 0);
        tl_cookie.to(cookie.bg, 0.8, { scaleY: 0, transformOrigin: "bottom left", ease: ease_custom.ease1 }, 0.6);
        tl_cookie.set(cookie.holder, { autoAlpha: 0, className: '+=remove' }, 1);
        ...
        var menu = {
            ...
            item: jQuery("#menu-holder .menu-a, #menu-holder .submenu-a"), //('#menu-holder .menu-item')
        ...
        var splitsTexts = [];
        var itemD = 0;
        var menu_tl = new TimelineMax({ paused: true, reversed: false });

        menu_tl.set(menu.holder, { autoAlpha: 0, visibility: 'hidden' })
            .set(menu.bg, { scaleY: 0, transformOrigin: "top right" })
            .set(menu.holder, { autoAlpha: 1 }, 0)
            .to(menu.bg, 1.2, { scaleY: 1, ease: ease_custom.ease1 }, 0.1)

        for (var i = 0; i <= menu.item.length - 1; i += 1) {
            splitsTexts.push(new SplitText(menu.item[i], {
                type: "chars"
            }));
            /*splitsTexts[i].chars.sort(function () {
        ...
        menu_tl.staggerTo(menu.fade, 1, { alpha: 1, ease: ease_custom.ease1 }, 0.02, 0.9)

            /*Banner*/
            .set(menu.menuBanner, { autoAlpha: 1 }, .9)
            .to(menu.bannerImageMask, 1.2, { scaleY: 0, transfromOrigin: "bottom left", ease: ease_custom.ease3, rotation: 0 }, .95)
            .to(menu.bannerImage, 1.2, { scale: 1, alpha: 1, transfromOrigin: "center center", ease: ease_custom.ease1, rotation: 0 }, .95)
            .staggerTo(menu.bannerFadeTop, 1, { y: 0, alpha: 1, ease: ease_custom.ease3 }, 0.15, 1);
        ...
        if (document.getElementById('home-video')) {
            ...
            var tl_expand_video = new TimelineMax({ paused: true, reversed: false });
            tl_expand_video.to(videoExpand.fade, .7, { autoAlpha: 0, ease: ease_custom.ease1 }, 0);
            tl_expand_video.to(videoExpand.holder, 1.4, { width: "100%", ease: ease_custom.ease1 }, .5);
            tl_expand_video.to(videoExpand.rightBox, 1.4, { width: "0%", ease: ease_custom.ease1 }, .5);
            tl_expand_video.to(videoExpand.close, 1.4, { autoAlpha: 1, ease: ease_custom.ease1 }, 1.4);
            ...
            videoExpand.close.click(function () {
                tl_expand_video.timeScale(1.5).reverse().eventCallback("onReverseComplete", function () {
                    TweenLite.set(videoExpand.holder, { clearProps: 'all' });
                    TweenLite.set(videoExpand.rightBox, { clearProps: 'all' });
        ...
        if ($body.hasClass('page-template-gallery')) {
            ...
            if ($window_w >= 768) {
                var tl_gallery = new TimelineMax({ paused: true, reversed: false, onComplete: updateCursor });
                tl_gallery.staggerTo(gallery.gFadeout, 1.2, { autoAlpha: 0, ease: ease_custom.ease1 }, 0.2, 0)
                tl_gallery.to(gallery.gLeft, 1.6, { y: "-100%", autoAlpha: 0, ease: Expo.easeIn }, 0.3)
                tl_gallery.to(gallery.gMiddle, 1.6, { y: "100%", autoAlpha: 0, ease: Expo.easeIn }, 0.35)
                tl_gallery.to(gallery.gRight, 1.6, { y: "-100%", autoAlpha: 0, ease: Expo.easeIn }, 0.3)
                tl_gallery.to(gallery.vertical, 1.6, { autoAlpha: 0, ease: Expo.easeIn }, 0.4)
                tl_gallery.set(gallery.horizontal, { autoAlpha: 1, visibility: "visible" }, 2.5)
                tl_gallery.to(gallery.mask, 1.8, { scaleY: 0, ease: ease_custom.ease3 }, 2)
                tl_gallery.to(gallery.image, 1.6, { scale: 1, transformOrigin: "center center", ease: ease_custom.ease1 }, 2)

                function updateCursor() {
                    TweenMax.to(gallery.cursor, 1.2, { autoAlpha: 1, ease: ease_custom.ease1 })
                    gallery.header.mouseenter(function () {
                        TweenMax.to(gallery.cursor, 1.2, { autoAlpha: 0, ease: Expo.easeOut })
                    });
                    gallery.header.mouseleave(function () {
                        TweenMax.to(gallery.cursor, 1.2, { autoAlpha: 1, ease: Expo.easeOut })
                    });
                }
        ...
        if ($window_w > 767) {
            if ($body.hasClass('page-template-timeline')) {
                ...
                var tl_timeline = new TimelineMax({ paused: true, reversed: false });
                tl_timeline.staggerTo(timeLine.tlFadeout, 1.2, { autoAlpha: 0, ease: ease_custom.ease1 }, 0.2, 0)
                tl_timeline.to(timeLine.gLeft, 1.6, { y: "-100%", autoAlpha: 0, ease: Expo.easeIn }, 0.3)
                tl_timeline.to(timeLine.gMiddle, 1.6, { y: "100%", autoAlpha: 0, ease: Expo.easeIn }, 0.35)
                tl_timeline.to(timeLine.gRight, 1.6, { y: "-100%", autoAlpha: 0, ease: Expo.easeIn }, 0.3)
                tl_timeline.to(timeLine.vertical, 1.6, { autoAlpha: 0, ease: Expo.easeIn }, 0.4)
        ...
        var tl_cat = new TimelineMax({ paused: true, reversed: false });
        tl_cat.set($body, { className: "+=overflow" });
        tl_cat.set(categories.holder, { autoAlpha: 0 });
        tl_cat.set(categories.bg, { scaleY: 0, transformOrigin: "top left" });

        tl_cat.set(categories.holder, { autoAlpha: 1 }, 0);
        tl_cat.to(categories.bg, 1.2, { scaleY: 1, transformOrigin: "top left", ease: ease_custom.ease1 }, 0.2);
        tl_cat.staggerTo(categories.tb, 1.2, { y: "0%", ease: ease_custom.ease1 }, 0.2, 0.4);
        tl_cat.staggerTo(categories.fadeIn, 1.2, { alpha: 1, ease: ease_custom.ease1 }, 0.15, 0.4);
        tl_cat.to(categories.close, 1.2, { alpha: 1, ease: ease_custom.ease1 }, 0.2, 0.8);
        tl_cat.to(categories.btn, 1.2, { alpha: 1, ease: ease_custom.ease1 }, 1.2);
        ...
        /*NEWSLETTER*/
        var tl_ns = new TimelineMax({ paused: true });
        ...
        tl_ns.set($body, { className: "+=overflow" });
        tl_ns.set(ns.holder, { autoAlpha: 0 });
        tl_ns.set(ns.bg, { scaleY: 0, transformOrigin: "top right" });

        tl_ns.set(ns.holder, { autoAlpha: 1 }, 0);
        tl_ns.to(ns.bg, 1.2, { scaleY: 1, transformOrigin: "top right", ease: ease_custom.ease1 }, 0.2);
        tl_ns.staggerTo(ns.fade, 1, { autoAlpha: 1 }, 0.2, 1);
        ...
        video_tl.set($body, { className: "+=overflow" })
            .set(video.container, { autoAlpha: 0 })
            .set(video.jsFade, { autoAlpha: 0 })

            .to(video.container, 0.01, { autoAlpha: 1 }, 0)
            .to(video.bg, 1, { scaleX: 1.2, transformOrigin: "top right", ease: ease_custom.ease3 })
                ...
                keyframe: function (element, name, direction) {
                    ...
                    if ($content.hasClass('in-view') && name == "dataBottomTop" && direction == "down") {
                        ...
                        if ($content.hasClass('js-zoom-out')) {
                            ...
                            tl_anim.to(jQuery(element), trans, { scale: 1, alpha: 1, transfromOrigin: "center center", ease: ease }, delay);
                        ...
                        if ($content.hasClass('js-mask-reveal')) {
                            ...
                            tl_anim.to(jQuery(element).find('.js-mask-reveal__mask'), 1.6, { scaleY: 0, ease: ease_custom.ease3 }, delay);
                            tl_anim.to(jQuery(element).find('.js-mask-reveal__img'), 1.3, { scale: 1, transformOrigin: "center center", ease: ease_custom.ease1 }, delay);
                        }
                        ...
                        if ($content.hasClass('js-zoom-r')) {
                            ...
                            tl_anim.to(jQuery(element).find('.js-zoom-r__holder'), 1.24, { scale: 1, transfromOrigin: "center center", ease: ease, rotation: 0 }, delay);
                            tl_anim.to(jQuery(element).find('.js-zoom-r__img'), 1.2, { scale: 1, alpha: 1, transfromOrigin: "center center", ease: ease, rotation: 0 }, delay);
                        }
                        ...
                        if ($content.hasClass('js-letters')) {
                            ...
                            var content = new SplitText(jQuery(element).find('.js-letters__title'), { type: "chars lines", linesClass: "overflow" }),
        ...
        jQuery("#gen_inq_button").click(function () { jQuery("#gen_inq_button").hasClass("active") || (jQuery("#gen_inq_button").addClass("active"), jQuery("#rec_desk_button").removeClass("active"), 
            jQuery("#gen_inq_button span").addClass("btn-full--bl color--team").removeClass("btn--black btn--h-cream "), jQuery("#rec_desk_button span").addClass("btn--black btn--h-cream ").removeClass("btn-full--bl color--team"), jQuery("#reception_form").fadeOut(function () { jQuery("#inquiry_form").fadeIn() })) }), jQuery("#rec_desk_button").click(function () { jQuery("#rec_desk_button").hasClass("active") || (jQuery("#rec_desk_button").addClass("active"), jQuery("#gen_inq_button").removeClass("active"), jQuery("#rec_desk_button span").addClass("btn-full--bl color--team").removeClass("btn--black btn--h-cream "), jQuery("#gen_inq_button span").addClass("btn--black btn--h-cream ").removeClass("btn-full--bl color--team"), jQuery("#inquiry_form").fadeOut(function () { jQuery("#reception_form").fadeIn() })) })
    ...
    checkform_ns: function (formid, $reply_holder) {
        ...
        $this_form.find(".required-chk").each(function () {
            if (!jQuery(this).prop("checked")) {
                ...
                show_error(id, formid);
                jQuery($reply_holder).html('<div class="reply-error color--red">' + $this_form.data('error_mandatory') + '</div>');
                //jQuery($reply_holder).html('<div class="reply-error color-red">'+$this_form.data('error_terms')+'</div>');
                //return false;
            }
        });
